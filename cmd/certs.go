...
func getSSLConfig() (x509Certs []*x509.Certificate, rootCAs *x509.CertPool, tlsCert *tls.Certificate, secureConn bool, err error) {
	if !(isFile(credentials.GetPublicCertFile()) && isFile(credentials.GetPrivateKeyFile())) {
		return nil, nil, nil, false, nil
	}

	if x509Certs, err = parsePublicCertFile(credentials.GetPublicCertFile()); err != nil {
		return nil, nil, nil, false, err
	}

	var cert tls.Certificate
	if cert, err = tls.LoadX509KeyPair(credentials.GetPublicCertFile(), credentials.GetPrivateKeyFile()); err != nil {
		return nil, nil, nil, false, err
	}

	tlsCert = &cert

	if rootCAs, err = getRootCAs(credentials.GetCADir()); err != nil {
		return nil, nil, nil, false, err
	}
